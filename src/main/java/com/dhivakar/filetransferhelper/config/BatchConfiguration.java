...
public class BatchConfiguration {
    ...
    public Step readAndCopyFiles() {
        ...
        return this.stepBuilderFactory.get("Read and Copy Files").<BatchInfo, ImportRecord>chunk(5).reader(customDataReader)
                .processor(filesProcessor).writer(compositeItemWriter).build();
    }

    @Bean
    public Job analyzeAndCopyLatestFilesJob(JobBuilderFactory jobBuilderFactory, JobCompletionListener listener) {

        return jobBuilderFactory.get("Analyze and Copy Latest Files").incrementer(new RunIdIncrementer())
                .listener(listener).start(readAndCopyFiles()).build();
    }
